...
//!   `kad_bootstrap_endpoints(position, endpoint)`. Dev-phase legacy setting values
    ...
    match target {
        ...
        15 => {
            tx.execute_batch(
                ...
                CREATE TABLE IF NOT EXISTS kad_bootstrap_endpoints (
...
mod tests {
    ...
    fn upgrade_from_v3_preserves_user_data_and_reaches_current() {
        ...
        for (table, column) in [
            ...
            ("settings", "section"),
            ("settings", "key"),
            ("kad_bootstrap_endpoints", "endpoint"),
        ] {
